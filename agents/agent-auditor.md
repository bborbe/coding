...
2. **Resolve and read the agent file** - Resolve the input argument to a file: if it has no `/`, look in `agents/` (project), then `~/.claude/agents/` (user-global), then `~/.claude/plugins/marketplaces/*/agents/` (plugin); append `.md` if missing. Then read the complete content with line numbers.
