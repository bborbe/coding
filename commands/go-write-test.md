...
/go-write-test basic
/go-write-test standard

# Test specific paths
/go-write-test basic .
/go-write-test standard pkg/user
/go-write-test integration myservice/
```
...
if [ -z "$PATH_ARG" ]; then
  ...
  if [ -z "$MODIFIED_FILES" ]; then
    ...
    echo ""
    echo "Suggestions:"
    echo "  /go-write-test $MODE .           # Test all files in current directory"
    echo "  /go-write-test $MODE pkg/        # Test specific package"
    exit 0
  fi
...
Use the Task tool to spawn `go-test-writer-assistant` agent:
