...
### RULE go-linting/complexity-limits-enforced (MUST)
...
```yaml
# .golangci.yaml — no complexity gate
linters:
  enable:
    - revive
    - errcheck
...
### RULE go-linting/banned-packages-via-depguard (MUST)
...
```yaml
# .golangci.yaml — depguard not configured
linters:
  enable:
    - revive
    - errcheck
