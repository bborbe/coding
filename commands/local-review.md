...
6. **Diff anchoring** — in PR mode (the scope class local-review shares with pr-review), report security findings ONLY on diff-changed lines or invariants whose attack surface the diff touched; whole-file context is permitted for reasoning, never for gating.
