...
│   └── service_suite_test.go     # Ginkgo test suite setup
```

### Test Suite Setup

Create `<package>_suite_test.go` in your package root.

**File naming rule:** Always prefix with the package name (`<package>_suite_test.go`):

- ✅ `lib_suite_test.go`, `service_suite_test.go`, `delivery_suite_test.go`
- ❌ `suite_test.go` (no package prefix)

The package prefix makes the file unambiguous when searching across a multi-package repo and matches the convention used throughout the codebase.
...
- `//go:generate go run -mod=mod github.com/maxbrunsfeld/counterfeiter/v6 -generate` in `<package>_suite_test.go`
...
│   │   ├── handler.go
│   │   └── handler_test.go
│   └── service_suite_test.go  # Test suite with generate directive
...
- Single `<package>_suite_test.go` with `//go:generate` directive
