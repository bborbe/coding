...
- docs: say plainly that the mechanical funnel's scope is changed **files**, not changed **lines** — in both `commands/pr-review.md` § 4a and the runner's own header. The runner is handed a file list and scans each file whole; it is never given the base ref, so it cannot narrow to the diff's hunks, and a changed file therefore drags in every pre-existing violation it already carried. The word "diff-scoped" in the command and in the script header read as line-scope, so the findings looked like a defect in the run rather than the expected shape of a file-scoped scan, and the adjudicator had no instruction to filter them. Measured 2026-09-26 on a 232-line Go diff: 15 findings across 4 owners, every one on an unchanged line — a pre-existing `glog` import, counterfeiter `Returns` calls, in-memory `range` loops and untyped enum constants, all refuted at once by `git diff origin/master...HEAD | grep -E "^[+-].*(glog|IsLiveReturns|ResolveReturns|range items)"` returning nothing. The command now states that checking each finding against the diff is the adjudicator's job, and the header no longer claims a scope it does not implement
- fix: run the citation validator from the plugin root in `docs/selector-mode-guide.md` § Step 4d-sel. `validate-citations.sh` resolves `rules/index.json` from the cwd or an ancestor, so the invocation the guide documented — run from the repo under review — exits 2 with `ERROR: rules/index.json not found. Run from repo root.` on every repo except this plugin's own checkout, a message that reads like a broken install rather than the wrong working directory. Reproduced and fixed 2026-09-26: old form from `bborbe/attention-controller` → exit 2; new form from the same cwd → exit 0. ⚠️ The first attempt to verify this ran the old form from *this* repo, where `rules/index.json` sits at the cwd — a false negative that would have retired a real defect
...
- commit: scope commits with an explicit `--` pathspec instead of `git add .`, so unrelated dirty files are never swept in
...
- feat: bench report page — each scored configuration gets a committed page at `bench/reports/<config_hash>.md` with Configuration, Runs, Per-PR, and Gap-triage candidates sections; a finding matching no golden entry is reported as a gap-triage candidate, **not a precision failure**, because the golden set is bootstrapped from one strong-model run and sharpens through disagreement
- feat: bench CLI surface — `--golden` now scores the live run instead of rejecting it with exit 2; new `--score` mode scores an existing ledger offline; `--reports-dir` defaults to `bench/reports`; per-row `PRS VERSION SKIP`; new exit-2 preconditions for `GOLDEN SET NOT FOUND`, `INVALID GOLDEN SET`, `GOLDEN VERSION MISMATCH`, `EMPTY LEDGER`, `CORRUPT LEDGER`, `INVALID CONFIG HASH`
- docs: bench README — document the scoring contract (match rule, three states, gap-triage classification, occurrence-index run chunking, report location and section order, three-decimal ratio rendering and `n/a` literal, absence of generation timestamp, committed-output invariant now naming the two exceptions: report pages under `bench/reports/` and frozen ledger slices under `bench/testdata/`); add four ledger slices to the fixtures table with `sha256` digests
...
- fix: Accept a `-ts` / `-tsx` / `-js` suffix on a detector's `id:` in `check-coverage.sh`, so language variants keep unique ast-grep ids while mapping to one documented rule
