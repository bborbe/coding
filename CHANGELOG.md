...
- feat: Wire the `--security` flag into `commands/code-review.md` — position-independent flag parsing (`SECURITY_REVIEW=1`), a Security mode subsection that mirrors the canonical pr-review wiring for audit scope (whole-repo recon and model under `/tmp/security-model.json`, no diff anchoring, six classifier trait groups with deterministic invariant selection, verifier-gated high-severity findings, derived blocking, fail-closed whole-repo deps pass, `SECURITY_MODEL_FILE` citation validation), and a baseline-independent `Security Findings` whole-repo inventory + `Security Model` provenance block — by referencing the frozen pipeline guide, selector-mode guide, verifier agent, and polymorphic validator rather than re-implementing them
...
- feat: Wire the `--security` flag into `commands/local-review.md` — position-independent flag parsing (`SECURITY_REVIEW=1`, recognized as a flag, never consumed as the directory path), a Security mode subsection that mirrors the canonical pr-review wiring for the local scope (recon and adjudicator over `git diff HEAD~1`, session-local model under `/tmp/security-model.json` mirroring `/tmp/local-review-findings.json`, six classifier trait groups with deterministic invariant selection, verifier-gated high-severity findings, derived blocking, PR-mode diff anchoring on diff-changed lines, fail-closed deps pass over local manifests, `SECURITY_MODEL_FILE` citation validation), and `Security Findings` + `Security Model` report sections — by referencing the frozen pipeline guide, selector-mode guide, verifier agent, and polymorphic validator rather than re-implementing them
- feat: Finalize the three draft security-mode acceptance scenarios (007 IDOR-confirmed, 008 IDOR-rejected-by-verifier, 009 toolchain-fail-closed) and author 010 (security-zero-findings) — each builds its generic Go fixture inline in Setup (two-commit diff on a local-only branch under `$WORK`, no perpetual fixture PR, no TBD), walks `/coding:local-review --security` in a fresh Claude Code session, and asserts the verifier verdict fields (`"confidence"` / `reject_reason` / `counterevidence_checked` / `"blocking"`), the fail-closed validator behavior, and that `security-model.json` never lands in the fixture repo; all four stay `status: draft`, not promoted to active
